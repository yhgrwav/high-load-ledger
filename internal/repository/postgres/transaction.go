...
func (db *Repository) CreateTransaction(ctx context.Context, tx entity.CustomTx, tr *entity.Transaction) error {
	query := `INSERT INTO ledger.transactions (id, user_from, user_to, currency, amount, idempotency_key, created_at)
...
func (db *Repository) GetTransactionByID(ctx context.Context, id uuid.UUID) (*entity.Transaction, error) {
	...
	err := db.pool.QueryRow(ctx, query, id).Scan(&tr.ID, &tr.FromAccountID, &tr.ToAccountID, &tr.Currency, &tr.Amount, &tr.IdempotencyKey, &tr.CreatedAt)
	if err != nil {
